...
- 文档中的 Agent Run 状态、字段和 API 与已经实现的 P1 后端不一致。
...
P1 Mock Runner 明确标注为临时进程内实现，不能被描述为生产运行时。
...
P1 Agent Run 契约保持当前已实现字段和 `202` 行为，不在文档对齐任务中制造破坏性变更。
...
P1 已实现的扁平字段暂时兼容。进入 `runtime_events` 阶段时，再通过专项迁移设计转为
稳定信封，不能直接破坏现有前端。
...
P1 ChatPage 保留当前 Workbench 视觉样式，新增 `/chat`，不重做 Dashboard。开发环境通过
...
- P0：已完成，允许补充验证和文档修正。
- P1：后端 Agent Run Mock 已完成，前端 ChatPage 联调进行中。
- P2-P16：未开始，除非专项契约明确标注已有部分实现。
...
5. 对比 P1 文档与 Pydantic Schema、路由、Model 和 Migration。
